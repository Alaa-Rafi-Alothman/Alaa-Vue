  ...
  <v-container>
    <v-row>
      <v-col cols="12" md="8" class="mx-auto">
        ...
        <h3 class="mb-4">Alaa</h3>

        <v-text-field v-model="newTask" label="Add a new task"
          @keyup.enter="addTask"
        ...
        <v-list>
          <v-list-item v-for="(task, idx) in filteredTasks"  :key="task.id" class="d-flex align-center">
            <v-text-field v-if="editIndex === idx" v-model="editText" @keyup.enter="saveEdit(idx)" @blur="saveEdit(idx)"
              dense
              hide-details
              autofocus>
            </v-text-field>

           <span @dblclick="startEdit(idx, task.text)" style="flex:1; cursor:pointer;">
              {{ task.text }}
            </span>
...
export default {
  ...
  computed: {
    filteredTasks() {
      ...
      return this.tasks.filter(task => task.text.toLowerCase().includes(this.filterText.toLowerCase()));
    },
  },
